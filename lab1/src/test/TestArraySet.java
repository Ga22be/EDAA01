...
public class TestArraySet {
	...
	public final void testManyDifferent() {
		for (int i = 1; i <= 1000; i++) {
			assertTrue("Duplicate found in set:" + i, s.add(i));
		}
	...
	public final void testManyDuplicates() {
		...
		for (int j = 2; j <= 10; j++) {
			for (int i = 1; i <= 1000; i++) {
				assertFalse("Duplicate not found in set:" + i, s.add(i));
			}
		}
		assertEquals("Wrong size():", 1000, s.size());
		for (int i = 1; i <= 1000; i++) {
	...
	public final void testAddAll() {
		...
		assertTrue("3 wasn't added accordingly to addAll instruction", s.addAll(t));
	}
	
	@Test
	public final void testAddAllIdentical() {
		ArraySet<Integer> t = new ArraySet<Integer>();
		t.add(1);
		t.add(2);
		...
		assertFalse("Objects were still added to s", s.addAll(t));
	}
}
